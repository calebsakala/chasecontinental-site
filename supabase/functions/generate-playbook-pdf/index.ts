...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const addPageFooter = (pageNum: number) => {
      ...
      doc.text("Chase Continental", pageWidth - margin, pageHeight - 10, { align: "right" });
    };
    ...
    doc.text("CHASE CONTINENTAL", margin, 25);
    ...
    doc.setTextColor(...GRAY);
    doc.setFont("helvetica", "normal");
    doc.text("of digital initiatives", margin + boxWidth/2, y + 23, { align: "center" });
    doc.text("meet targets (Gartner)", margin + boxWidth/2, y + 28, { align: "center" });
    ...
    doc.setFont("helvetica", "bold");
    doc.text("$4T", margin + boxWidth + 10 + boxWidth/2, y + 15, { align: "center" });
    doc.setFontSize(9);
    doc.setTextColor(...GRAY);
    doc.setFont("helvetica", "normal");
    doc.text("DX spend by 2027", margin + boxWidth + 10 + boxWidth/2, y + 23, { align: "center" });
    doc.text("(IDC)", margin + boxWidth + 10 + boxWidth/2, y + 28, { align: "center" });
    ...
    const quote = '"Transformations fail not because the idea is wrong, but because execution breaks when reality hits."';
    ...
    y += 5;
    const execContent2 = 'By starting with workflows (not tools), designing for edge cases upfront, and focusing on outcomes, organizations can achieve the "Digital Vanguard" status where 71% of initiatives succeed (Gartner). This guide is for CIOs, CxOs, and transformation leaders ready to move from pilots to production.';
    ...
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...DARK);
    doc.text("Book a call with Chase Continental to implement this playbook.", margin + 5, y + 14);
    ...
    const tocItems = [
      ...
      { num: "2", title: "The 5 Reasons AI Pilots Fail in Production", page: "5" },
      { num: "3", title: "Workflow Selection Checklist: What to Automate First", page: "7" },
      { num: "4", title: 'The "Exceptions + Approvals" Design Pattern', page: "8" },
      { num: "5", title: "Implementation Checklist: From Build to Rollout", page: "9" },
      { num: "6", title: "Measurement: Proving ROI and Reliability", page: "10" },
    ...
    const section1Intro = "Digital transformation is accelerating, but results are uneven. According to Gartner's 2025 CIO Survey:";
    ...
    y += 5;
    const section1Content2 = "IDC forecasts DX spending at a 16.2% CAGR, hitting nearly $4T by 2027 – potentially two-thirds of all ICT spend. Fastest growth in financial services (20.5% CAGR) and discrete manufacturing ($700B+ by 2027).";
    ...
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...DARK);
    doc.text("Without co-ownership and robust execution, investments risk failure. This playbook bridges the gap.", margin + 30, y + 8);
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text("Most transformations fail due to execution gaps, not ideas. Here's why – and how to fix it:", margin, y);
    y += 12;
    
    const failures = [
      {
        reason: "Ignoring Edge Cases",
        problem: "Pilots work on clean data but break on real-world exceptions (e.g., messy inputs).",
        solution: "Map exceptions upfront using stakeholder interviews. Design adaptive logic (e.g., if-then rules for 80/20 coverage).",
      },
      {
        reason: "Tool-First Approach",
        problem: "Starting with AI hype without workflow fit leads to mismatches.",
        solution: "Audit workflows first; select tools second. Use a compatibility matrix.",
      },
      {
        reason: "Lack of Approvals Integration",
        problem: "Automations halt on human decisions, causing bottlenecks.",
        solution: "Embed approval gates with notifications (e.g., Slack/Email triggers).",
      },
      {
        reason: "Poor Adoption",
        problem: "Teams resist due to complexity or lack of training.",
        solution: "Involve end-users in design; provide change management playbooks.",
      },
      {
        reason: "No Outcome Metrics",
        problem: "Success is vague, leading to abandoned projects.",
        solution: "Define KPIs pre-launch (e.g., time saved, error reduction); track post-rollout.",
      },
    ];
    ...
    failures.forEach((item, i) => {
      ...
      const probLines = doc.splitTextToSize(`Problem: ${item.problem}`, contentWidth - 12);
      doc.text(probLines, margin + 12, y);
      y += probLines.length * 5 + 3;
      
      // Solution
      doc.setTextColor(...TEAL);
      const solLines = doc.splitTextToSize(`Prevention: ${item.solution}`, contentWidth - 12);
    ...
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...DARK);
    doc.text("43% of CIOs are decreasing legacy infra spend for cloud/AI experiments (Gartner) – but without these fixes, waste ensues.", margin + 28, y + 7);
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text("Prioritize workflows with high impact and low complexity. Use this checklist:", margin, y);
    y += 12;
    
    // Checklist table
    const checklistItems = [
      { criterion: "Repetitive & Rule-Based: Does it involve manual data entry or decisions?", points: "+2" },
      { criterion: "High Volume: Processes 100+ instances/week?", points: "+2" },
    ...
    checklistItems.forEach((item, i) => {
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(9);
      const criterionLines = doc.splitTextToSize(item.criterion, contentWidth - 35);
    ...
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...DARK);
    doc.text("8+ = Automate Now  |  5-7 = Pilot  |  <5 = Defer", margin + 5, y + 13);
    y += 25;
    
    // Example
    doc.setTextColor(...GRAY);
    doc.setFont("helvetica", "italic");
    doc.text("Example: Automate claims processing (financial services) – 35.1% CAGR growth potential (IDC).", margin, y);
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text("Reality hits with exceptions (e.g., invalid data) and approvals (e.g., manager sign-off). This pattern ensures resilience:", margin, y);
    y += 12;
    
    const steps = [
      { title: "Step 1: Map Core Flow", desc: "Document the standard path from start to finish." },
      { title: "Step 2: Identify Exceptions", desc: 'Brainstorm 10-20 scenarios (e.g., "Missing field? Route to human.").' },
      { title: "Step 3: Embed Approvals", desc: "Use API triggers for escalations (e.g., auto-pause until approved)." },
      { title: "Step 4: Test Iteratively", desc: "Simulate 100 runs; refine for 95%+ automation rate." },
    ];
    ...
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...DARK);
    doc.text("Reduces failure from 52% (average) to Vanguard levels (Gartner).", margin + 23, y + 7);
    ...
    doc.setFontSize(18);
    doc.setTextColor(...DARK);
    doc.text("Implementation Checklist: From Build to Rollout", margin + 18, y + 5);
    y += 15;
    ...
    const phases = [
      { phase: "Build Phase", items: ["Assemble cross-functional team (IT + Business).", "Prototype in low-code tools."] },
      { phase: "Test Phase", items: ["Unit tests for exceptions.", "User acceptance testing (UAT)."] },
      { phase: "Rollout Phase", items: ["Phased deployment (e.g., 10% users first).", "Training sessions."] },
    ...
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...DARK);
    doc.text("Dedicate 26% of business staff to tech work, as in Vanguard orgs (Gartner).", margin + 15, y + 7);
    ...
    const metrics = [
      { name: "Reliability", target: "Uptime >98%; Exception handling rate <5%." },
      { name: "ROI", target: "Time saved × Hourly rate - Implementation costs. Aim for 6-12 month payback." },
      { name: "Adoption", target: "User satisfaction score >4/5; Usage rate >80%." },
      { name: "Expansion Metrics", target: "Number of scaled workflows post-pilot." },
    ];
    ...
    metrics.forEach((metric) => {
      ...
      doc.text(targetLines, margin + 8 + doc.getTextWidth(metric.name + ": "), y);
      y += 10;
    });
    ...
    doc.setTextColor(...GRAY);
    doc.text("Prove value amid $351B IT spend (Gartner) and $4T DX market (IDC).", margin + 5, y + 14);
    ...
    const whyCards = [
      { title: "We Start with the Workflow, Not the Tool", desc: "Ensures fit, avoiding 80% of mismatches." },
      { title: "We Design for Edge Cases Upfront", desc: "Handles reality, boosting success to 71% (Vanguard)." },
      { title: "We Measure Outcomes, Then Expand", desc: "Data-driven scaling, aligning with 80% CIO investment priorities (Gartner)." },
    ];
    ...
    const nextSteps = [
      { num: "1", title: "Assess", desc: "Use the checklist to select a high-priority workflow." },
      { num: "2", title: "Pilot", desc: "Apply the design pattern in a controlled pilot." },
      { num: "3", title: "Scale", desc: "Measure outcomes and book a call for expansion." },
    ];
    ...
    y += 6;
    doc.text("• Chase Continental Resources: chasecontinental.com/resources", margin, y);
    ...
    doc.setTextColor(255, 255, 255);
    doc.setFont("helvetica", "bold");
    doc.text("Ready to implement this playbook?", margin + contentWidth/2, y + 10, { align: "center" });
    doc.setFont("helvetica", "normal");
    doc.text("Contact: info@chasecontinental.com", margin + contentWidth/2, y + 18, { align: "center" });
    ...
    doc.setTextColor(...DARK);
    doc.setFont("helvetica", "bold");
    doc.text("Transform with confidence.", pageWidth / 2, y, { align: "center" });
    
    y += 20;
    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...GRAY);
    doc.text("Thank you for downloading the AI Transformation Playbook.", pageWidth / 2, y, { align: "center" });
    ...
    doc.setTextColor(255, 255, 255);
    doc.setFont("helvetica", "bold");
    doc.text("Download more at chasecontinental.com/resources", pageWidth/2, y + 9, { align: "center" });
    ...
    const { error: uploadError } = await supabase.storage
      .from("lead-magnets")
      .upload("transformation-playbook/AI-Transformation-Playbook.pdf", pdfBuffer, {
